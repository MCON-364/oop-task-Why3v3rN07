...
class DemoMainTest {
    ...
    void testDemonstrateAddingTasks() {
        ...
        Task doc = testRegistry.get("Write documentation").orElseThrow(()->new TaskNotFoundException("Write documentation task should exist"));
        assertEquals(Priority.HIGH, doc.priority(), "Write documentation should be HIGH priority");

        Task review = testRegistry.get("Review pull requests").orElseThrow(()->new TaskNotFoundException("Review pull requests task should exist"));
        assertEquals(Priority.MEDIUM, review.priority(), "Review pull requests should be MEDIUM priority");

        Task dependencies = testRegistry.get("Update dependencies").orElseThrow(()->new TaskNotFoundException("Update dependencies task should exist"));
        assertEquals(Priority.LOW, dependencies.priority(), "Update dependencies should be LOW priority");

        Task bug = testRegistry.get("Fix critical bug").orElseThrow(()->new TaskNotFoundException("Fix critical bug task should exist"));
        assertEquals(Priority.HIGH, bug.priority(), "Fix critical bug should be HIGH priority");

        Task refactor = testRegistry.get("Refactor code").orElseThrow(()-> new TaskNotFoundException("Refactor code task should exist"));
    ...
    void testDemonstrateRetrievingTask() {
        ...
        // Retrieve it
        Task retrieved = testRegistry.get("Fix critical bug").orElseThrow(() -> new TaskNotFoundException("Retrieved task should not be null"));
    ...
    @Test
    @DisplayName("Retrieving non-existent task should return an empty Optional")
    void testDemonstrateRetrievingNonExistentTask() {
        TaskRegistry testRegistry = new TaskRegistry();

        Optional<Task> missing = testRegistry.get("Non-existent task");

        assertTrue(missing.isEmpty(), "Non-existent task should be empty");
    }
    ...
    void testDemonstrateUpdatingTask() {
        ...
        Task before = testRegistry.get("Refactor code").orElseThrow(()-> new TaskNotFoundException("Refactor code task should exist"));
        ...
        Task after = testRegistry.get("Refactor code").orElseThrow(()-> new TaskNotFoundException("Refactor code task should still exist after update"));
    ...
    void testDemonstrateUpdatingNonExistentTask() {
        ...
        // After refactoring to throw a custom exception, we expect TaskNotFoundException to be thrown
        assertThrows(TaskNotFoundException.class, () -> {
            testManager.run(new UpdateTaskCommand(testRegistry, "Non-existent task", Priority.HIGH));
        }, "Updating non-existent task should throw TaskNotFoundException (after custom exception refactoring)");
    ...
    void testUpdateTaskCommand() {
        ...
        Task updated = testRegistry.get("Test task").orElseThrow(()-> new TaskNotFoundException("Task should still exist after update"));
    ...
    void testTaskManagerRunWithUpdateCommand() {
        ...
        Task updated = testRegistry.get("Test task").orElseThrow(() -> new TaskNotFoundException("Task should still exist"));
